...
namespace ValueMapperUtility
{
    public static class ValueMapper
    {
            ...
            where TDestination : new()
        {
            ...
            if (array.Length < PAR)
            {
                // Optimization: pre-allocate result list and get mapping func once
                ...
                {
                    var dest = new TDestination();
                    map(item, dest, ignoredProperties);
                    result.Add(dest);
                }
                return result;
            }
        ...
        private static Func<object, object> CreateConverter(Type src, Type dst)
        {
            ...
            if (dst == StringType && src.IsEnum) return v => v?.ToString();
            if (dst.IsEnum && src == StringType)
            {
                // Return a converter that will throw for invalid values
                return v => v == null ? GetDefault(dst) : Enum.Parse(dst, (string)v, true);
            }
            if (dst.IsEnum && IsNumeric(src)) return v => v == null ? GetDefault(dst) : Enum.ToObject(dst, v);
